...
subtest 'BookNoFK => AuthorNoPK' => sub
{
...

	{
		...
		ok( not( defined( eval{ $bf -> push_anything_appropriate( author => $af ); $bf } ) ), 'Impossible connection is impossible' );

		my $old_field = $af -> returning_field();

		$af -> returning_field( $af -> model() -> borrow_field( 'id' ) );

		$bf -> push_anything_appropriate( author => $af );

		$af -> returning_field( $old_field );

		ok( not( defined( eval{ $bf -> push_anything_appropriate( author => $af ); $bf } ) ), 'Impossible connection is still impossible' );
	}
...
subtest 'Author => BookNoFK' => sub
{
...

	{
		...
		$bf -> push_anything_appropriate( id => $af );
...
subtest 'Author => BookNoFKAndNoPK' => sub
{
...

	{
		...
		ok( not( defined( eval{ $bf -> push_anything_appropriate( id => $af ); $bf } ) ), 'Impossible connection is impossible' );
		...
		ok( not( defined( eval{ $bf -> push_anything_appropriate( id => $af ); $bf } ) ), 'Impossible connection is still impossible' );
	}
